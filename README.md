...
[`docs/project/canonical-decisions.md`](docs/project/canonical-decisions.md) for the
full set of architecture decisions.
...
The suite runs against SQLite `:memory:` by default (see `stockflow/phpunit.xml`). A
handful of tests need real MySQL semantics (FULLTEXT indexes, real unique
constraints, real row locking, real Redis serialization) and switch to a dedicated
`stockflow_testing` database at runtime via the
`Tests\Concerns\UsesRealMysqlDatabase` trait — they never touch the dev database.
Notably:
...
`stockflow/app/Services/StockService.php` is the only place `stock_movements` /
...
`stock.transfer` request touches (`stockflow/app/Support/WarehouseAccess.php`);
SuperAdmin bypasses team scoping. See `docs/project/canonical-decisions.md` §3/§6
...
Payment methods (`stockflow/app/Payments/*Gateway.php`, resolved by
...
`EXPLAIN` plans) lives in
[`stockflow/docs/technical/cache.md`](stockflow/docs/technical/cache.md) and
[`stockflow/docs/technical/indexing.md`](stockflow/docs/technical/indexing.md).
Summary:
...
- [x] Cache invalidation — flush-on-write for both, documented in `stockflow/docs/technical/cache.md`
- [x] Laratrust permission cache — self-invalidating; stale-cache-after-reset bug fixed + regression-tested
- [x] MySQL indexes — inventoried with real `EXPLAIN` plans in `stockflow/docs/technical/indexing.md`
...
- `stockflow/routes/web.php` — Inertia page routes only (session/`web` guard). No
  API routes.
- `stockflow/routes/api.php` — external Passport B2B API only, mounted as `/api/v1`.
- `stockflow/routes/webhooks.php` — `/webhooks/v1/paymob`, `/webhooks/v1/fawry`,
  and `/webhooks/v1/fake-gateway`; mounted without the web/session/CSRF stack and
  signature-verified where the provider supports it.
- `stockflow/app/Http/Controllers/Web/` — Inertia controllers, one folder per module
  (`Auth/`, `Admin/`, `Catalog/`, `Stock/`, `Sales/`, `Procurement/`, `Import/`,
  `Reports/`).
- `stockflow/app/Http/Controllers/Api/V1/` — JSON API controllers for external
  B2B/system clients; thin wrappers around the same Services used by Web
  controllers.
- `stockflow/app/Http/Controllers/Webhooks/` — Paymob/Fawry/Fake Gateway webhook
  endpoints.
...
- `stockflow/resources/js/Pages/` — Inertia pages, resolved by
  `resources/js/app.jsx`, one folder per module (`Auth/`, `Admin/`, `Catalog/`,
  `Stock/`, `Sales/`, `Storefront/`, `Procurement/`, `Import/`, `Reports/`).
- `stockflow/resources/js/Layouts/` — `AppLayout` (authenticated shell, renders
  `FlashMessage`), `GuestLayout` (centered card, used by auth pages), and
  `StorefrontLayout` (public catalog/cart shell).
- `stockflow/resources/js/Components/` — shared UI primitives: `Button`, `Input`,
  `Select`, `Table`, `Pagination`, `Breadcrumbs`, `FlashMessage`,
  `PermissionGate`.
- `stockflow/app/Http/Middleware/HandleInertiaRequests.php` — shares `auth.user`,
  `auth.roles`, `auth.permissions`, and `flash.success`/`flash.error` on every
  Inertia response.
- `stockflow/app/Console/Commands/` — `stock:reconcile` and
